...
//boleto borrar_boleto(int EventoSeleccionado, int asientos_total, evento un_evento[]);
...
int menu_boletos(int evento_seleccionado, int boletos_disponibles, int eventos_creados, int evento_numero_maximo, evento un_evento[], int cantidad_entradas){
    int opcion;
    int  seleccion, i, save; //save = es una funcion que guradara el valor de la "cantidad_entradas -1" (para la funcion de borrar/cancelar_venta_voleto)
    char sino;
    ...
    while (opcion!=4)
    {
        ...
        printf("\n*________________________________________________________________________________________________________*");
   	 	...
   	 	switch(opcion)
   	 	{
                ...
                if (sino=='s' || sino=='S')
                {
                    save=cantidad_entradas;
                    for ( i = 0; seleccion < cantidad_entradas ; i++)
                    {
                        ...
                        cantidad_entradas--; // Es la unica forma que encontre para que el siclo for funcionara
                    }
                    cantidad_entradas=save-1; //Recuperamos las catidade perdidas por el ciclo for
                }
                else
                {
