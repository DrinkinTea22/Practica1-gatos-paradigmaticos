...
void Ordenar_evento_burbuja_asc (evento un_evento[max_eventos], boleto MatrizBoleto[max_eventos][numero_asientos]);
void Ordenar_evento_burbuja_desc (evento un_evento[max_eventos], boleto MatrizBoleto[max_eventos][numero_asientos]);
   ...
   scanf("%d", &numero_asientos);
    ...
    do
    {
        ...
        case 5:
            ...
            printf("Ingrese una opcion: ");
            scanf("%d", &opcion_ordenamiento);
            if (opcion_ordenamiento == 1)
                Ordenar_evento_burbuja_asc (un_evento, MatrizBoleto);
            else if (opcion_ordenamiento == 2)
                Ordenar_evento_burbuja_desc (un_evento, MatrizBoleto);
            else
                printf("\nOpcion incorrecta.\n");
            break;
        case 6:
        ...
        for(i=0;i<num_eventos;i++){
            for ( j=i+1; j < num_eventos; j++)
            {
                if (un_evento[i].cantidad_entradas>un_evento[j].cantidad_entradas)
                {
                    ...
                    //reemplazo entre espacios del vector
                    ...
                    for (k = 0; k < numero_asientos; k++)
                    {
                        ...
                        //reemplazo entre espacios de LA MATRIZ
        ...
        for(i=0;i<num_eventos;i++){
            for ( j=i+1; j < num_eventos; j++)
            {
                if (un_evento[i].cantidad_entradas<un_evento[j].cantidad_entradas)
                {
                    ...
                    //reemplazo eentre espacios del vector
                    un_evento[i].cantidad_entradas=un_evento[j].cantidad_entradas;
                    un_evento[i].numero=un_evento[j].numero,
                    strcpy(un_evento[i].nombre,un_evento[j].nombre);
                    ...
                    for (k = 0; k < numero_asientos; k++)
                    {
                        ...
                        //reemplazo eentre espacisos de LA MATRIZ
                        strcpy(MatrizBoleto[i][k].tipo_entrada, MatrizBoleto[j][k].tipo_entrada);
                        MatrizBoleto[i][k].asiento = MatrizBoleto[j][k].asiento;
                        MatrizBoleto[i][k].precio = MatrizBoleto[j][k].precio;
                        strcpy(MatrizBoleto[i][k].evento, MatrizBoleto[j][k].evento);
